...
export default class FeedbackLoop {
    ...
    async _loop() {
        ...
        try {
            ...
            // Проверяем, что страница существует и подключена
            if (!this.page) {
            ...
            // Проверяем подключение через try/catch
            try {
                // Простая проверка: пытаемся выполнить скрипт
                await this.page.evaluate(() => document.readyState);
            } catch (e) {
                this.logger.warn('⚠️ Страница не доступна');
                this._scheduleNext();
                return;
            }
            
            // Получаем состояние GUI
            ...
            // Не логируем ошибку каждый раз, чтобы не засорять консоль
