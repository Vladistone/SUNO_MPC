...
export default class DeviceRouter {
    ...
    // ============================================================
    // ИТОГОВЫЙ УНИВЕРСАЛЬНЫЙ КОНВЕЙЕР ДИСПЕТЧЕРИЗАЦИИ КОМАНД
    // ============================================================
    _dispatchCommand(command) {
        if (!command) return;

        const map = this.device?.mappingTable;

        // --- НИЗКОУРОВНЕВАЯ ТРАНСЛЯЦИЯ ЧЕРЕЗ MAPPING TABLE КОНТРОЛЛЕРА ---
        // Если протокол hui прислал сырую пару зона/функция, сопоставляем её с картой устройства
        if (command.protocol === 'hui' && command.zone !== undefined && map) {
            // 1. Сначала ищем в глобальной секции маппинга (например, зона 10 навигации)
            let targetType = map.global?.[command.zone]?.[command.function]?.type;
            
            // 2. Если в глобальной нет, определяем динамический режим порта и ищем в секции modes
            if (!targetType) {
                const activePortConfig = this.device?.ports?.[command.port];
                const currentMode = activePortConfig?.mode || this.mode;
                
                // Перехватываем тип кнопки на основе текущего режима матрицы (track или fx)
                const buttonTarget = map.modes?.[currentMode]?.buttons?.[command.function];
                if (buttonTarget) {
                    targetType = buttonTarget.type;
                    if (buttonTarget.index) command.sendIndex = buttonTarget.index;
                }
            }

            if (targetType) {
                command.type = targetType;
                // Защита от дублирования логов: отсекаем отпускание (off) для системной навигации
                if (command.state === 'off' && (targetType.startsWith('bank') || targetType.startsWith('chan'))) {
                    return; 
                }
            }
        }
        ...
        // Эти кнопки меняют живые смещения в памяти и поглощаются драйвером
        switch (command.type) {
            case 'bank_next':
        ...
        // --- ДИНАМИЧЕСКИЙ ПЕРЕХВАТ НЕПРЕРЫВНЫХ ЭЛЕМЕНТОВ ПО МАТРИЦЕ РЕЖИМОВ ---
        if (map && (command.type === 'fader' || command.type === 'vpot')) {
            const activePortConfig = this.device?.ports?.[command.port];
            const currentMode = activePortConfig?.mode || this.mode;
            const elementTarget = map.modes?.[currentMode]?.elements?.[command.type];
            if (elementTarget) {
                command.type = elementTarget.type;
            }
        }
        ...
        if (command.channel !== undefined && command.port) {
            const activePortConfig = this.device?.ports?.[command.port];
            const defaultOffset = command.port === 'right' ? 8 : 0;
            const offset = activePortConfig?.offset !== undefined ? activePortConfig.offset : defaultOffset;
        ...
        if (command.type === 'select') {
            // Реагируем строго на физическое нажатие кнопки (state === 'on')
            if (command.state === 'on') {
                this.selectedTrack = command.channel;
                
                // Переключаем триггер режима: если был track -> станет fx, и наоборот
                this.mode = (this.mode === 'track') ? 'fx' : 'track';
            ...
            return; // Поглощаем команду select, чтобы она не сбивала микшер при навигации
        }
        ...
        if (command.channel !== undefined) {
            ...
            // Записываем состояние в оперативную память драйвера
            switch (command.type) {
                ...
                // Фиксируем специфичные значения для FX-матрицы в chStates
                case 'fx_send_level': this.chStates[ch].fx[`send${command.sendIndex || 1}`] = command.value; break;
                case 'fx_param': this.chStates[ch].fx.param1 = command.value; break; 
            }
        }
        
        // Отправляем очищенную команду с финальным виртуальным каналом наверх в core/index.js
